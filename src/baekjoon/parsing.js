...
import { updateProblemsFromStats } from '../common/store/storage';
import { unescapeHtml } from '../common/utils/util';
import {
  RESULT_CATEGORY,
  languages,
  categories,
...
export async function parseData(doc = document) {
  ...
  const details = await makeDetailMessageAndReadme(
    problemId,
    submissionId,
    language,
    memory,
...
export async function findDatas(datas) {
  datas = datas.filter(
    (data) => !isNaN(Number(data.problemId)) && Number(data.problemId) > 1000,
  ); // 대회 문제 제외
  const details = await findProblemsInfoAndSubmissionCode(
    datas.map((x) => x.problemId),
    datas.map((x) => x.submissionId),
  );
...
export function makeDetailMessageAndReadme(data) {
  ...
  const directory = `백준/${level.replace(
    / .*/,
    '',
...
export function parseProblemDescription(doc = document) {
  ...
  const problem_input =
    doc.getElementById('problem_input')?.innerHTML.trim?.().unescapeHtml?.() ||
    'Empty'; // eslint-disable-line
  const problem_output =
    doc.getElementById('problem_output')?.innerHTML.trim?.().unescapeHtml?.() ||
    'Empty'; // eslint-disable-line
 ...
 * @returns {Promise<Array>}
 */
export async function findProblemsInfoAndSubmissionCode(
  problemIds,
  submissionIds,
) {
  if (debug) console.log('in find with promise');
  ...
  return Promise.all([problemDescriptions, Codes, SolvedAPI]).then(
    ([problemDescriptions, codeTexts, solvedJsons]) => {
